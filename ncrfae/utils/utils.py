...
############ Liwen Zhang Implement ############

# noinspection PyUnresolvedReferences
def set_seed(random_seed):
    random_seed = hash_string(random_seed) if isinstance(random_seed, str) else random_seed
...
def sign_smoothing(cnts, alpha):
    return (np.array(cnts) != 0).astype(float)

def uniform_smoothing(cnts, alpha):
    m, n = len(cnts), len(cnts[0])
    return np.ones((m, n)) / n

######### pretrained embedding #########

def load_embedding(path, datasets):
...
def norm_embedding(train, *other_datasets):
    holder = []
    for instance in train:
        holder.append(instance.r_embed)
    std = torch.std(torch.cat(holder, dim=0))

    for instance in train:
        instance.r_embed = instance.r_embed / std

    for dataset in other_datasets:
        for instance in dataset:
            instance.r_embed = instance.r_embed / std

def split_discourse(ins: DataInstance, inst_r_embed=None):
    pbnds = ins.pbnds
    ...
    for idx, (span_begin, span_end) in enumerate(new_spans):
        ...
        if inst_r_embed is not None:
            r_embed = torch.cat([inst_r_embed[0].unsqueeze(0), inst_r_embed[span_begin: span_end+1]], dim=0)
        else:
            r_embed = None
        ...
        for s_begin, s_end in ins.sbnds:
            if span_begin-1 <= s_begin <= s_end <= span_end-1:
        ...
        # consider shift
        shift = edu_ids[0] - 1
        shifted_edu_ids = [i for i in range(len(edu_ids)+1)]
        shifted_edus = [ins.edus[0]] + edus
        shifted_edus_postag = [ins.edus_postag[0]] + edus_postag
        shifted_edus_head = [ins.edus_head[0]] + edus_head
        shifted_name = name
        ...
        head_edu = datainstance.edus[1:]
        head_pos = [ins[1] for ins in datainstance.edus_head][1:]
        heads = [ins[0] for ins in datainstance.arcs]
        scidtb_inst = SCIDTBSentence(head_edu, head_pos, heads=heads, sbnds=shifted_sbnds, raw=datainstance)
        if r_embed is not None:
            scidtb_inst.r_embed = r_embed
        new_inst_list.append(scidtb_inst)
