...
class Model(nn.Module):
    ...
    def __init__(self, args, embedding):
        ...
        self.args = args
        n_words, word_embed = embedding.size()
        self.word_embed = nn.Embedding(num_embeddings=n_words, embedding_dim=word_embed)
        self.load_pretrained(embedding)
        self.embed_dropout = IndependentDropout(p=args.embed_dropout)

        # the word-lstm layer
        self.lstm = BiLSTM(input_size=word_embed*2,
    ...
    def load_pretrained(self, embed=None):
        if embed is not None:
            self.word_embed = nn.Embedding.from_pretrained(embed)
    ...
    def real_forward(self, sentences):
        ...
        for sentence in sentences:
            tmp_len.append(len(sentence.raw.edus))
            begin_idx.append(torch.tensor([edu[0] for edu in sentence.idx]))
            end_idx.append(torch.tensor([edu[-1] for edu in sentence.idx]))
        ...
        begin_idx = pad_sequence(begin_idx, batch_first=True, padding_value=0).to(self.mlp_arc_d.linear.weight.device)
        end_idx = pad_sequence(end_idx, batch_first=True, padding_value=0).to(self.mlp_arc_d.linear.weight.device)

        begin_embed = self.word_embed(begin_idx)
        end_embed = self.word_embed(end_idx)
        embed = self.embed_dropout(torch.cat([begin_embed, end_embed], dim=-1))[0]
...
class DocLSTMBiaffine(nn.Module):

    def __init__(self, args, embedding):
        super(DocLSTMBiaffine, self).__init__()
        ...
        # the word-lstm layer
        self.doc_lstm = BiLSTM(input_size=word_embed,
                           hidden_size=args.n_lstm_hidden,
                           num_layers=args.n_lstm_layers,
                           dropout=args.lstm_dropout)

        self.lstm = BiLSTM(input_size=args.n_lstm_hidden*4,
                           hidden_size=args.n_lstm_hidden,
                           num_layers=args.n_lstm_layers,
                           dropout=args.lstm_dropout)
        self.lstm_dropout = SharedDropout(p=args.lstm_dropout)
    ...
    def real_forward(self, sentences):
        ...
        end_pos = []
        for sentence in sentences:
            sentence_idx = [i for edu in sentence.idx for i in edu]
            doc_batch.append(torch.tensor(sentence_idx))
            doc_length.append(len(sentence_idx))
            begin_pos.append(torch.tensor(sentence.begin_pos))
            end_pos.append(torch.tensor(sentence.end_pos))
        doc_max_len = max(doc_length)

        doc_len_tp = torch.tensor(doc_length)
        doc_idx = pad_sequence(doc_batch, batch_first=True, padding_value=0).to(self.mlp_arc_d.linear.weight.device)
        doc_embed = self.embed_dropout(self.word_embed(doc_idx))[0]

        doc_x = pack_padded_sequence(doc_embed, doc_len_tp, True, False)
        doc_x, tmp = self.doc_lstm(doc_x)
        doc_x, _ = pad_packed_sequence(doc_x, True, total_length=doc_max_len)
        doc_x = self.lstm_dropout(doc_x)
        tmp_len = []


        begin_pos = pad_sequence(begin_pos, batch_first=True, padding_value=0)
        end_pos = pad_sequence(end_pos, batch_first=True, padding_value=0)

        begin_embed = []
        end_embed = []
        for idx in range(len(sentences)):
            begin_embed.append(doc_x[idx, begin_pos[idx]])
            end_embed.append(doc_x[idx, end_pos[idx]])
        begin_embed = torch.stack(begin_embed, dim=0)
        end_embed = torch.stack(end_embed, dim=0)

        embed = torch.cat([begin_embed, end_embed], dim=-1)
        for idx, sentence in enumerate(sentences):
            tmp_len.append(len(sentence.idx))
        self.seq_len = max(tmp_len)

        lens = torch.tensor(tmp_len)
        mask = (torch.arange(self.seq_len)[None, :] < lens[:, None]).to(self.mlp_arc_d.linear.weight.device)
        # embed = pad_sequence(embed, batch_first=True, padding_value=0.0)
        x = pack_padded_sequence(embed, lens, True, False)
        x, _ = self.lstm(x)
        x, _ = pad_packed_sequence(x, True, total_length=self.seq_len)
        x = self.lstm_dropout(x)
...
class BiaffineAE(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.word_embed = nn.Embedding(num_embeddings=args.n_words, embedding_dim=args.word_embed)

        self.embed_dropout = IndependentDropout(p=args.embed_dropout)

        # the word-lstm layer
        self.lstm = BiLSTM(input_size=args.embed_dim,
                           hidden_size=args.n_lstm_hidden,
                           num_layers=args.n_lstm_layers,
                           dropout=args.lstm_dropout)
        self.lstm_dropout = SharedDropout(p=args.lstm_dropout)
